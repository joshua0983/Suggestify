...
def extract_item_features(item_properties):
    """Extract item features from the item_properties DataFrame."""
    start_time = time.time()
...
def calculate_item_popularity(events):
    """Calculate item popularity based on events data."""
    start_time = time.time()
...
def combine_features(item_features, item_popularity):
    """Combine item features and popularity into a single DataFrame."""
    start_time = time.time()
